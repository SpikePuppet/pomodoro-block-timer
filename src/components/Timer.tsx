...
export function Timer({
  ...
  return (
    <div className={`timer timer--${phase}`}>
      <TomatoMascot phase={phase} status={status} size={100} />
      ...
      <div className="timer__pomodoro-count">
        {Array.from({ length: 4 }).map((_, i) => (
          <span 
            key={i} 
            className={`timer__dot ${i < (pomodorosCompleted % 4) ? 'timer__dot--filled' : ''}`}
          >
            🍅
          </span>
        ))}
      ...
      <div className="timer__total">
        Total completed: {pomodorosCompleted} pomodoro{pomodorosCompleted !== 1 ? 's' : ''}
      </div>
    </div>
  );
}
